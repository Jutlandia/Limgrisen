...
use serenity::all::{CommandData, CommandInteraction, PartialChannel, User, UserId};
use serenity::builder::{
    CreateChannel, CreateCommand, CreateCommandOption, CreateMessage, EditChannel,
};
...
async fn handle_create(
    ...
    if let Ok(ctf) = Ctf::fetch_by_snowflake(pool, channel_id).await {
        ...
        let category = match Category::from(tmp_cat.to_string()) {
            Category::Other(cat) => {
                if options.len() == 3 {
                    ...
                    return "Option `other_type` is required for Category `Other`".to_string();
                }
            }
            default => default,
        };
        ...
        let act_cat =
            sqlx::query!(r#"SELECT value FROM config WHERE option = 'Active Challenges'"#)
                .fetch_one(pool)
                .await
                .unwrap()
                .value
        ...
        let chan_name = format!("{}-{}-{}", &ctf.name, &category.to_string(), &name).to_string();
        ...
        match guild_channel {
            Ok(chan) => {
                let challenge =
                    Challenge::create(pool, name.to_string(), category, &ctf, chan.id).await;
...
async fn handle_done(
    ...
    let completed_cat =
        sqlx::query!(r#"SELECT value FROM config WHERE option = 'Completed Challenges'"#)
            .fetch_one(pool);
    ...
    match challenge_res.await {
        Ok(challenge) => {
            ...
            if let Some(chall_chan) = challenge.channel_id() {
                ...
                let completed_id =
                    match ChannelId::from_str(completed_cat.await.unwrap().value.unwrap().as_str())
                    {
                        Ok(res) => res,
                        Err(err) => {
                            println!("{:#?}", err);
            ...
            if let Some(ctf_channel) = challenge.ctf.channel_id() {
                match ctf_channel
                    .send_message(&ctx.http, CreateMessage::new().content(&credit))
                    .await
                {
                    Ok(_) => {}
                    Err(err) => {
    ...
    CreateCommand::new("challenge")
        ...
        .set_options(vec![
            ...
            .set_sub_options(vec![
                ...
                .add_string_choice("Reverse Engineering", Category::ReverseEngineering)
                .add_string_choice("Other", Category::Other("other".to_string())),
                CreateCommandOption::new(
                    CommandOptionType::String,
                    "name",
